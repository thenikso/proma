...
const arnRegexp =
	/^arn:aws:(?<operation>[^:]+):(?<region>[^:]+):(?<account>[^:]+):(?<apiId>[^\/]+)\/(?<stage>[^\/]+)\/(?<httpMethod>[^\/]+)(?<route>.+)$/i;
...
export const handler = async (event) => {
	...
	const { aud: audience, sub: userId, permissions } = await getJwtClaims(token, key);
	...
	return allowPolicy(event.methodArn, decodedJwt.payload.sub, decodedJwt.payload.permissions);
};
