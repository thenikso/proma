...
export const endpoint = async (event) => {
	...
	if (endpoints.length === 0) {
		releaseLogCapture();
		return error(404, `Can not find endpoint ${hostId}/${projectSlug}/endpoints/${endpoint}.*`);
	}
	...
	try {
		...
		const chipCompiledClass = await getCompiledClass(chipClass.compile(), chipClass.imports);
	...
	let chipResult;
	try {
		chipResult = await Promise.race([deferred.promise, timeout(5000, new Error('timeout (5s)'))]);
	} catch (e) {
		releaseLogCapture();
		return error(400, e);
	}
