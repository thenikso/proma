...
import { createShortcutDispatcher, shortcuts, action } from '@proma/svelte-components';

//
// Shortcuts
//

shortcuts.set('!cmd+S', action.all(action('CurrentProject.save'), action('Playground.save')));
...
shortcuts.set('[ChipBoard:port] alt+click', action('ChipBoard.removeConnection'));
...
export function initShortcuts() {
	...
	onMount(() => {
		...
		return () => {
			document.removeEventListener('keydown', preventDefaultShortcutsCaptured, true);
