 ...
 * @typedef {(portInstance: unknown, scope: CompilationScope, codeWrapper: unknown, tools?: unknown) => unknown} PortCompiler
 * @typedef {{
 *   compile: (port: unknown, scope: CompilationScope, codeWrapper: unknown) => unknown,
 *   compiler: (portInfo: PortInfo) => PortCompiler,
 *   isOutlet: (port: unknown, scope: CompilationScope) => boolean,
 *   getConnectedPorts: (port: unknown, scope: CompilationScope) => unknown[],
 *   getPortAndChipInstance: (port: unknown, scope: CompilationScope) => { port: unknown, chip: unknown, parentChip: unknown },
 * }} CompilerCtx
 */
 ...
 * @param {CompilerCtx} ctx
 * @returns {PortCompiler}
 */
export function computeCompiler(
  portInfo,
  prop = 'compute',
  ...
  portInfo[compilerProp] = function compileCompute(
    ...
    return astBuilder({
      ...
      compileInputData(/** @type {string} */ portName) {
        ...
        return ctx.compile(port, [chip, ...scope], codeWrapper);
      },
  ...
  portInfo[compilerProp] = function compileExecute(
    ...
    return astBuilder({
      ...
      compileOutputData(
        ...
        if (ctx.isOutlet(port, outterScope)) {
          const body = [];
        ...
        for (const conn of conns) {
          if (conn) {
            ...
            if (conn.chip && conn.chip !== scope[0]) {
              outputIdentifier = ctx.compile(
                conn,
                [conn.chip, ...scope],
                codeWrapper,
              );
            } else {
              outputIdentifier = ctx.compile(conn, scope, codeWrapper);
            }
 ...
 * @returns {PortCompiler}
 */
export function makeOutputFlowSinkCompiler(portInfo, ctx) {
  assert(
  ...
  return function findSourceCompiler(portInstance, outterScope, codeWrapper) {
    ...
    if (ctx.isOutlet(portInstance, outterScope)) {
    ...
    if (portInstance.variadic) {
      ...
      for (const vPort of portInstance.variadic) {
        ...
        // identifier.
        let vFuncIdent = ctx.compile(vPort, [chip, ...scope], codeWrapper);
    ...
    if (portInfo.computeOutputs.size > 0) {
      ...
      for (const computePortInfo of portInfo.computeOutputs) {
        ...
        if (computePortInfo.compute) {
          assignBlock = computeCompiler(computePortInfo, 'compute', 'computeCompiler', ctx)(
            computePortInstance,
 ...
 * @returns {PortCompiler}
 */
export function makeInputFlowSourceCompiler(portInfo, ctx) {
  assert(
  ...
  if (portInfo.execute) {
    return function useExecuteCompiler(portInstance, outterScope, codeWrapper) {
      const executeExpression = executeCompiler(portInfo, 'execute', 'executeCompiler', ctx)(
        portInstance,
        outterScope,
        codeWrapper,
        ctx,
  ...
  return function forwardSourceCompiler(
    ...
    if (ctx.isOutlet(portInstance, outterScope)) {
      ...
      //    const then = outputFlow('then');
      //    wire(exec, then);
      if (ctx.isOutlet(conn, outterScope)) {
      ...
      return ctx.compile(conn, [conn.chip, ...outterScope], codeWrapper);
    }
 ...
 * @returns {PortCompiler}
 */
export function makeInputDataSinkCompiler(portInfo, ctx) {
  assert(
  ...
  return function forwardDataCompiler(portInstance, outterScope, codeWrapper) {
    ...
    if (conn) {
      if (conn.chip !== scope[0]) {
        return ctx.compile(conn, [conn.chip, ...scope], codeWrapper);
      }
      return ctx.compile(conn, scope, codeWrapper);
    }
 ...
 * @returns {PortCompiler}
 */
export function makeOutputDataSourceCompiler(portInfo, ctx) {
  assert(
    ...
    return function computeOnCompiler(portInstance, outterScope, codeWrapper) {
      ...
      if (ctx.isOutlet(portInstance, outterScope)) {
        ...
        if (ctx.isOutlet(conn, outterScope)) {
          ...
          assignExpressionBlock = ctx.compile(
            conn,
            conn.chip === chip
              ? [conn.chip, ...scope]
  ...
  return function useComputeCompiler(portInstance, outterScope, codeWrapper) {
    ...
    const makeCompute = portInfo.allowSideEffects
      ? executeCompiler(portInfo, 'compute', 'computeCompiler', ctx)
      : computeCompiler(portInfo, 'compute', 'computeCompiler', ctx);
