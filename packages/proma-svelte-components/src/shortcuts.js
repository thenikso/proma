...
shortcuts.set = function addGlobalShortcut(shortcut, action) {
	resolvedGlobalShortcuts.set(shortcut, resolveSingleShortcut(shortcut, action));
...
export function createShortcutDispatcher(targetSelectors = [], localShortcuts = {}) {
	...
	function dispatch(event, options) {
		return dispatchShortcuts(event, options, targetPathResolvers, resolvedLocalShortcuts);
	}

	Object.defineProperty(dispatch, 'localShortcuts', {
		get() {
			return Object.fromEntries(
				resolvedLocalShortcuts.map(({ shortcut, action }) => [shortcut, action]),
			);
		},
		set(value) {
...
function dispatchShortcuts(event, options, targetPathResolvers, localShortcuts) {
	// Match event
	let localMatches = [];
	if (localShortcuts) {
	...
	if (globalShortcutsMathers.length > 0) {
		globalMatches = globalShortcutsMathers.filter(({ matchEvent }) => matchEvent(event));
	}
	// Check for capture shortcuts
	if (options && typeof options.capture === 'boolean') {
		localMatches = localMatches.filter(({ capture }) => capture === options.capture);
		globalMatches = globalMatches.filter(({ capture }) => capture === options.capture);
	}
	...
	if (localMatches.length > 0) {
		anyDispatched =
			anyDispatched ||
			matchTargetAndDispatch(localMatches, [targetsPath[targetsPath.length - 1]], event);
	}
	if (globalMatches.length > 0) {
		...
		anyDispatched = anyDispatched || matchTargetAndDispatch(globalMatches, targetsPath, event);
	}
...
function makeSingleTargetResolver(selectors) {
	return function singleTargetResolver(event) {
		...
		for (let i = 0, l = selectors.length; i < l; i++) {
			...
			const target = typeof select === 'function' ? select(event, targetsIdsPath) : select;
			if (target) {
...
function makeTargetMatcher(targetsString) {
	const templateIds = targetsString.split(':').map((x) => x.split('|').map((x) => x.trim()));
...
function resolveSingleShortcut(shortcut, action) {
	const [, capturing, targetString, eventsString] = shortcutRegExp.exec(shortcut) || [];
	if (!eventsString) {
...
function makeShortcutMatcher(eventString) {
	...
	const combinationsMathcers = combinations.map((tokens) => {
		const mods = {
			metaKey: tokens.includes('meta') || tokens.includes('win') || tokens.includes('cmd'),
